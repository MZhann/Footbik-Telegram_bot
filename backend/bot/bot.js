...
(async function main() {
  ...
  const OPEN_APP = /^\/open_app(?:@[\w_]+)?(?:\s+([\s\S]+))?$/i;
  // Начинает полл
  bot.onText(CMD, async (msg, match) => {
    ...
    await Poll.create({
      ...
      createdBy: creator,
      status: "ACTIVE", // (optional, default already ACTIVE)
    });
  });
  ...
  bot.onText(INFO_CMD, async (msg, match) => {
    ...
    const toLabel = (v) =>
      escapeHtml(v?.username ? `${v.username}` : v?.name || `id:${v?.id}`);
    const list = (arr) =>
      arr.length ? arr.map((v) => `- ${toLabel(v)}`).join("\n") : "-";

    const text =
      `<b>Опрос:</b> ${escapeHtml(doc.title)}\n` +
      `<b>Статус:</b> ${doc.status === "CLOSED" ? "закрыт" : "открыт"}\n` +
  ...
  bot.onText(OPEN_APP, async (msg) => {
    ...
    await safeSendMessage(
      chatId,
      'https://t.me/footbik_god_bot/footbik'
    );
  });
  ...
  bot.on("poll_answer", async (answer) => {
    ...
    if (!poll) return;
    if (poll.status === "CLOSED") return; // guard
