...
router.get('/', async (req, res, next) => {
  try {
    const { chatId, status, limit = 20 } = req.query;
    const q = {};
    if (chatId) q.chatId = String(chatId);
    if (status && (status === 'ACTIVE' || status === 'CLOSED')) q.status = status;
...
router.get('/:chatId/:messageId', async (req, res, next) => {
  try {
    ...
    const doc = await Poll.findOne({ chatId: String(chatId), messageId: Number(messageId) });
